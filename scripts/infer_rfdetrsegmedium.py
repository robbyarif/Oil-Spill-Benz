...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--binary-oil",
        action="store_true",
        help="Remap classes to binary labels: oil vs not_oil.",
    )
    parser.add_argument(
        "--oil-class-names",
        type=str,
        default="oil,oils-emulsions,emulsion,sheen",
        help="Comma-separated COCO class names considered as oil when --binary-oil is enabled.",
    )
    parser.add_argument(
        "--oil-class-ids",
        type=str,
        default="",
        help="Comma-separated COCO class IDs considered as oil when --binary-oil is enabled.",
    )
...
def _parse_csv_ids(csv_text: str) -> List[int]:
    ids: List[int] = []
    for part in _parse_csv_names(csv_text):
        ids.append(int(part))
    return ids


def resolve_oil_class_ids(
    class_name_map: Dict[int, str],
    oil_class_names_csv: str,
    oil_class_ids_csv: str,
) -> List[int]:
    selected_ids = set(_parse_csv_ids(oil_class_ids_csv))
    selected_names = {name.lower() for name in _parse_csv_names(oil_class_names_csv)}

    for class_id, class_name in class_name_map.items():
        if class_name.lower() in selected_names:
            selected_ids.add(int(class_id))
...
def remap_to_binary(
    ...
    oil_class_ids: Sequence[int],
) -> np.ndarray:
    oil_set = set(int(class_id) for class_id in oil_class_ids)
    return np.array([1 if int(class_id) in oil_set else 0 for class_id in class_ids], dtype=np.int64)
...
def main() -> None:
    ...
    oil_class_ids = resolve_oil_class_ids(
        class_name_map=class_name_map,
        oil_class_names_csv=args.oil_class_names,
        oil_class_ids_csv=args.oil_class_ids,
    )

    output_class_name_map = class_name_map
    output_categories = categories
    if args.binary_oil:
        output_class_name_map = {0: "not_oil", 1: "oil"}
        output_categories = [
            {"id": 0, "name": "not_oil"},
            {"id": 1, "name": "oil"},
        ]
    ...
    for raw_path in image_paths_raw:
        ...
        if args.binary_oil:
            class_ids = remap_to_binary(class_ids=class_ids, oil_class_ids=oil_class_ids)
        ...
        items = prediction_to_json_items(
            ...
            class_ids=class_ids,
            scores=scores,
            class_name_map=output_class_name_map,
    ...
    summary = {
        ...
        "binary_oil": args.binary_oil,
        "oil_class_ids": oil_class_ids,
        "oil_class_names": [class_name_map[class_id] for class_id in oil_class_ids if class_id in class_name_map],
