...
class RFDETRTrainer(BaseTrainer):
    ...
    def load_model(self, weights=None):
        ...
        if weights is not None:
            ...
            self.model = RFDETRSegMedium(pretrain_weights=weights, device=self.device)
            self.model.optimize_for_inference()
            print(f"Model loaded from checkpoint: {weights}")
        else:
            # self.model = RFDETRSegNano(device=self.device)
            self.model = RFDETRSegMedium(device=self.device)
...
def main():
    ...
    trainer.load_model(weights="runs/lados_rfdetrsegmedium/checkpoint_best_regular.pth")
    ...
    trainer.test(
        ...
        dst="runs/lados_rfdetrsegmedium/test",
        file_name="test.txt",
        color_coded=True,
        log=True,
        save=True,
